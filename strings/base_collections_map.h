...
WINRT_EXPORT namespace winrt
{
    ...
    {
        return make<impl::multi_threaded_observable_map<K, V, std::map<K, V, Compare, Allocator>>>(
    ...
    Windows::Foundation::Collections::IObservableMap<K, V> multi_threaded_observable_map(
        std::unordered_map<K, V, Hash, KeyEqual, Allocator> && values)
    {
        return make<impl::multi_threaded_observable_map<K, V, std::unordered_map<K, V, Hash, KeyEqual, Allocator>>>(
            std::move(values));
    }
}
...
namespace std
{
    ...
    struct tuple_size<winrt::Windows::Foundation::Collections::IKeyValuePair<K, V>> : integral_constant<size_t, 2>
    {};
