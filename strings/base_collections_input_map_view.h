...
namespace winrt::impl
{
          ...
          map_view_base<scoped_input_map_view<K, V, Container>, K, V>
    {
        ...
        explicit scoped_input_map_view(Container const& values) :
            m_values(values)
        {}
    ...
    template <typename K, typename V, typename Container> auto make_input_map_view(Container&& values)
    {
        return make<input_map_view<K, V, Container>>(std::forward<Container>(values));
    }

    template <typename K, typename V, typename Container> auto make_scoped_input_map_view(Container const& values)
    {
...
WINRT_EXPORT namespace winrt::param
{
    template <typename K, typename V> struct map_view
    {
        ...
        map_view(interface_type const& values) noexcept :
            m_owned(false)
        {
        ...
        map_view(std::initializer_list<std::pair<K const, V>> values) :
            m_pair(impl::make_input_map_view<K, V>(std::map<K, V>(values)), nullptr)
        {}
    ...
    template <typename K, typename V> auto get_abi(map_view<K, V> const& object) noexcept
    {
        return *(void**)(&object);
    }
    ...
    template <typename K, typename V> struct async_map_view
    {
        ...
        async_map_view(interface_type const& values) noexcept :
            m_owned(false)
        {
    ...
    template <typename K, typename V> auto get_abi(async_map_view<K, V> const& object) noexcept
    {
        return *(void**)(&object);
    }
}
