...
namespace winrt::impl
{
    ...
    template <typename T, typename InputIt> auto make_scoped_input_vector_view(InputIt first, InputIt last)
    {
...
WINRT_EXPORT namespace winrt::param
{
    template <typename T> struct vector_view
    {
        ...
        vector_view(interface_type const& values) noexcept :
            m_owned(false)
        {
        ...
        vector_view(std::vector<value_type, Allocator>&& values) :
            m_pair(make<impl::input_vector_view<value_type, std::vector<value_type, Allocator>>>(std::move(values)), nullptr)
        {}

        template <typename Allocator>
        vector_view(std::vector<value_type, Allocator> const& values) :
            m_pair(impl::make_scoped_input_vector_view<value_type>(values.begin(), values.end()))
        {}
    ...
    template <typename T> auto get_abi(vector_view<T> const& object) noexcept
    {
        return *(void**)(&object);
    }
    ...
    template <typename T> struct async_vector_view
    {
        ...
        async_vector_view(interface_type const& values) noexcept :
            m_owned(false)
        {
    ...
    template <typename T> auto get_abi(async_vector_view<T> const& object) noexcept
    {
        return *(void**)(&object);
    }
}
