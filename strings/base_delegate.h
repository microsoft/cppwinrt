...
#pragma warning(disable : 4458) // declaration hides class member (okay because we do not use named members of base class)
    ...
    template <typename R, typename... Args> struct WINRT_IMPL_NOVTABLE variadic_delegate_abi : unknown_abi
    {
    ...
    template <typename R, typename... Args> struct WINRT_IMPL_EMPTY_BASES delegate_base : Windows::Foundation::IUnknown
    {
        ...
        delegate_base(winrt::weak_ref<O>&& object, LM&& lambda_or_method) :
            delegate_base(
                [o = std::move(object), lm = std::forward<LM>(lambda_or_method)](auto&&... args)
                {
                    if (auto s = o.get())
                    {
        ...
        template <typename O, typename LM>
        delegate_base(std::weak_ptr<O>&& object, LM&& lambda_or_method) :
            delegate_base(
                [o = std::move(object), lm = std::forward<LM>(lambda_or_method)](auto&&... args)
                {
                    if (auto s = o.lock())
...
{
    template <typename... Args> struct WINRT_IMPL_EMPTY_BASES delegate : impl::delegate_base<void, Args...>
    {
