...
// Pure helper functions extracted from type_resolver.cpp for testability.
// These functions depend on winmd::reader types and winrt base library
// but have NO dependency on DkmProcess or the VS debugger SDK.
...
// Requires winmd::reader types and winrt base types to be available
// (via winmd_reader.h and base_identity.h / base_implements.h)

namespace natvis_helpers
{
    using namespace winmd::reader;

    template <typename T>
    inline bool has_attribute(T const& row, std::string_view const& type_namespace, std::string_view const& type_name) noexcept
    {
    ...
    inline std::string get_full_name(TypeDef const& type)
    {
        return std::string(type.TypeNamespace()) + "." + std::string(type.TypeName());
    }

    inline winrt::guid get_guid(TypeDef const& type)
    {
    ...
    inline std::wstring format_guid(winrt::guid guid)
    {
    ...
    inline auto get_default_interface(TypeDef const& type)
    {
    ...
    inline auto calculate_sha1(std::vector<uint8_t> const& input)
    {
    ...
    inline winrt::guid generate_guid(GenericTypeInstSig const& type)
    {
        using namespace winrt::impl;
        constexpr winrt::guid namespace_guid = { 0xd57af411, 0x737b, 0xc042,{ 0xab, 0xae, 0x87, 0x8b, 0x1e, 0x16, 0xad, 0xee } };
