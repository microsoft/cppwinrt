...
#define WINRT_%_H
    ...
    template <typename... Args> [[nodiscard]] static finish_with wrap_open_file_guard(writer& w, Args&&... args)
    {
...
%    };
    ...
    static void write_forward(writer& w, TypeDef const& type)
    {
        ...
        w.write(format, bind<write_generic_typenames>(generics), remove_tick(type_name.name));
    }
    ...
    static void write_category(writer& w, TypeDef const& type, std::string_view const& category)
    {
        ...
        else
        {
            ...
            w.write(format, bind<write_generic_typenames>(generics), type, bind_list(", ", generics));
        }
    }
    ...
    static void write_guid(writer& w, TypeDef const& type)
    {
        ...
        if (empty(generics))
        {
            ...
            w.write(format, type, bind<write_guid_value>(guid), bind<write_guid_comment>(guid));
        }
        else
        {
            ...
            auto format = R"(    template <%> struct abi<%>
    {
        struct WINRT_IMPL_NOVTABLE type : inspectable_abi
        {
            ...
            w.write(format, bind<write_generic_typenames>(generics), type);
        }
        ...
        auto format = R"(    template <%> struct abi<%>
    {
        ...
        w.write(format, bind<write_generic_typenames>(generics), type, bind<write_abi_params>(signature));
    }
...
%    };
    template <> struct abi<@::%>
    {
        ...
        w.write(format, impl_name, bind_each<write_field_abi>(type.FieldList()), type_name.name_space, type_name.name, impl_name);
    }
    ...
    static void write_consume_declaration(writer& w, MethodDef const& method)
    {
        ...
        [[nodiscard]] auto %(auto_revoke_t, %) const;
            ...
            w.write(format, method_name, type, type, method_name, method_name, bind<write_consume_params>(signature));
        }
    }
...
%%%    };
    ...
    static void write_produce_upcall(writer& w, std::string_view const& upcall, method_signature const& method_signature)
    {
        if (method_signature.return_signature())
        {
            ...
            {
                w.write("std::tie(*__%Size, *%) = detach_abi(%(%));", name, name, upcall, bind<write_produce_args>(method_signature));
            }
            else
            {
                w.write("*% = detach_from<%>(%(%));",
        ...
        {
            w.write("%(%);", upcall, bind<write_produce_args>(method_signature));
        }
...
%};
        ...
        for (auto&& [interface_name, info] : get_interfaces(w, class_type))
        {
            ...
            {
                w.write(format, interface_name, interface_name, bind_each<write_dispatch_overridable_method>(info.type.MethodList()));
            }
        }
    }
    ...
    static void write_optimized_call_factory(writer& w, TypeDef const& type, TypeDef const& factory, method_signature const& signature)
    {
        ...
        else
        {
            ...
            w.write(format, type.TypeName(), factory_name, factory_name, get_name(signature.method()), bind<write_consume_args>(signature));
        }
    }
...
%    };
        ...
        for (auto&& [interface_name, info] : get_interfaces(w, type))
        {
            if (info.overridable && !info.base)
            {
                ...
                w.write(format, type_name, type_name, info.type, bind_each<write_consume_declaration>(info.type.MethodList()));
            }
        }
    }
...
%    };
    ...
    inline bool operator!=(% const& left, % const& right)%
    {
        ...
        struct complex_struct
        {
            complex_struct(writer& w, TypeDef const& type) :
                type(type), is_noexcept(!has_reference(type))
            {
            ...
            {
                {
                    ...
                    w.write(format, method_name, factory.second.type, factory.second.type, method_name);
                }
                ...
                if (is_opt_type)
                {
                    ...
                    w.write(format, method_name, method_name, bind<write_consume_params>(signature));
                }
                else
                {
                    auto format = R"(        [[nodiscard]] static auto %(auto_revoke_t, %);
)";
                    w.write(format, method_name, bind<write_consume_params>(signature));
                }
            }
        }
    }
        ...
        %(impl::call_factory_cast<%(*)(winrt::Windows::Foundation::IActivationFactory const&), %>([](winrt::Windows::Foundation::IActivationFactory const& f) { return f.template ActivateInstance<%>(); }))
    {
                    ...
                    w.write(format, type_name, type_name, type_name, type_name, type_name, type_name);
                }
                else
                {
...
%    };
        ...
        w.write(format, type_name, type_name, bind_each<write_static_declaration>(factories, type));
    }
