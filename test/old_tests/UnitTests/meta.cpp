...
template <typename T> using get_abi_t = decltype(get_abi(std::declval<T&>()));

template <typename T> using put_abi_t = decltype(put_abi(std::declval<T&>()));
...
namespace
{
    ...
    template <typename... T> struct list_size<impl::interface_list<T...>>
    {
