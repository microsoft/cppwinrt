...
TEST_CASE("uniform_in_params")
{
    ...
    Class{ single_threaded_map<hstring, hstring>(std::map<hstring, hstring>{ { L"test", L"test" } })
               .as<IIterable<IKeyValuePair<hstring, hstring>>>(),
           0,
           0 };
    ...
    REQUIRE(L"test" ==
            c.InIterablePair(single_threaded_map<hstring, hstring>(std::map<hstring, hstring>{ { L"test", L"test" } })));
    REQUIRE(L"test" == c.InAsyncIterable({ L"test" }).get());
    REQUIRE(
        L"test" ==
        c.InAsyncIterablePair(single_threaded_map<hstring, hstring>(std::map<hstring, hstring>{ { L"test", L"test" } })).get());
    REQUIRE(L"test" == c.InMap(single_threaded_map<hstring, hstring>(std::map<hstring, hstring>{ { L"test", L"test" } })));
    REQUIRE(L"test" ==
            c.InMapView(single_threaded_map<hstring, hstring>(std::map<hstring, hstring>{ { L"test", L"test" } }).GetView()));
    REQUIRE(L"test" ==
            c.InAsyncMapView(single_threaded_map<hstring, hstring>(std::map<hstring, hstring>{ { L"test", L"test" } }).GetView())
                .get());
