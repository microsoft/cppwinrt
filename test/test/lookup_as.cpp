...
TEST_CASE("LookupAs")
{
    auto map = single_threaded_map<hstring, Windows::Foundation::IInspectable>();
    map.Insert(L"uri", Uri(L"http://kennykerr.ca"));
    map.Insert(L"enum", box_value(AsyncStatus::Completed));
    map.Insert(L"underlying_type", box_value(static_cast<int32_t>(AsyncStatus::Error)));
    map.Insert(L"guid", box_value(guid_of<Uri>()));
    map.Insert(L"hstring", box_value(L"Test"));
    map.Insert(L"nullptr", nullptr);

    test(map);
    test(map.GetView());

    // Just making these helpers work inherited
    ValueSet vs;
