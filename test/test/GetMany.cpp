...
namespace
{
    template <typename T> IIterator<T> single_threaded_generator(std::vector<T>&& values = {})
    {
        ...
        {
            explicit generator_container(IIterator<T> const& first) :
                m_current(first)
            {
        ...
        {
            explicit generator(IIterator<T> const& first) :
                m_container(first)
            {}
