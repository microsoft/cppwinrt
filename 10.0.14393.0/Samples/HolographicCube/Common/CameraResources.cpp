...
)
{
    ...
    winrt::check_hresult(
        GetDXGIInterfaceFromObject(surface, IID_PPV_ARGS(put(resource)))
    );
    ...
    if (get(m_d3dBackBuffer) != get(cameraBackBuffer))
    {
        ...
        if (HolographicCube::NotEqual(m_d3dRenderTargetSize, currentSize))
        {
    ...
    if (get(m_d3dDepthStencilView) == nullptr)
    {
        ...
        winrt::check_hresult(
            device->CreateDepthStencilView(
                ...
                put(m_d3dDepthStencilView)
            )
        );
    }

    // Create the constant buffer, if needed.
    if (get(m_viewProjectionConstantBuffer) == nullptr)
    {
        ...
        winrt::check_hresult(
            device->CreateBuffer(
                &constantBufferDesc,
                nullptr,
                put(m_viewProjectionConstantBuffer)
            )
        );
    }
}
...
)
{
    ...
    if (context == nullptr || get(m_viewProjectionConstantBuffer) == nullptr || !viewTransformAcquired)
    {
    ...
    else
    {
        ...
        context->UpdateSubresource(
            get(m_viewProjectionConstantBuffer),
            0,
            nullptr,
...
)
{
    ...
    if (context == nullptr || get(m_viewProjectionConstantBuffer) == nullptr || m_framePending == false)
    {
        return false;
    }
    ...
    ID3D11Buffer * buffer = get(m_viewProjectionConstantBuffer);
